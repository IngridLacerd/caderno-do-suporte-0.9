...
app.post('/api/users', async (req, res) => {
  ...
  const { rows } = await pool.query(
    'INSERT INTO users(id,name,avatar,color) VALUES($1,$2,$3,$4) ON CONFLICT(id) DO UPDATE SET name=$2,avatar=$3,color=$4 RETURNING *',
    [id, name, avatar || 'ti-user-circle', color || '#880000']
  );
  res.json(rows[0]);
});
