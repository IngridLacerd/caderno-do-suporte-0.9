...
function getToken() { return localStorage.getItem('suporte_token') || ''; }
...
async function apiFetch(path, opts) {
  try {
    ...
    if (res.status === 401) { localStorage.removeItem('suporte_token'); location.href = 'login.html'; return; }
    var data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Erro na API');
    return data;
  } catch(e) {
    if (e.message !== 'Redirecting') console.error('API Error', path, e);
    throw e;
  }
}
...
async function loadUsers() {
  // Não é mais necessário carregar lista de usuários — auth é por token
  users = [];
}

function loadSession() {
  // Sessão é mantida pelo token JWT no localStorage
  // currentUser é preenchido pelo /api/me no init
}
...
async function changePassword() {
  ...
  var password2 = document.getElementById('new-password-confirm') ? document.getElementById('new-password-confirm').value : '';
  if (!password || !password2) { showToast('Digite e confirme a nova senha.'); return; }
  if (password !== password2)  { showToast('As senhas não conferem.'); return; }
  if (password.length < 6)     { showToast('Senha deve ter pelo menos 6 caracteres.'); return; }
  try {
    await apiFetch('/api/me/password', { method: 'PUT', body: JSON.stringify({ password }) });
    closePasswordModal();
...
document.addEventListener('DOMContentLoaded', async function() {
  ...
  try {
    // Buscar dados do usuário atual e dados compartilhados
    var results = await Promise.all([
      apiFetch('/api/me'),
      apiFetch('/api/procedures'),
      apiFetch('/api/categories'),
    ...
    procs         = results[1];
    cats          = results[2].length ? results[2] : DEFAULT_CATS.slice();
    accessHistory = results[3];
    alerts        = results[4];
  } catch(e) {
